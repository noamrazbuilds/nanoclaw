...
2. Per-group default: read `/workspace/agent/tts-config.json` if it exists
3. System default: `local`

If user says "set default voice to X", write `/workspace/agent/tts-config.json`:
```json
...
```bash
OUTPUT_DIR="/workspace/agent/audio"
mkdir -p "$OUTPUT_DIR"
...
```bash
CHAT_JID=$(cat /workspace/agent/tmp/current_chat_jid 2>/dev/null || echo "$NANOCLAW_CHAT_JID")
cat > "/workspace/agent/tmp/messages/speak-$(date +%s%N).json" << EOF
{
  "type": "audio",
  "chatJid": "$CHAT_JID",
