...
## v2 hook point(s)
**Resolution: REPLACE v1's port with upstream skill — apply `upstream/skill/channel-formatting`.** Specifically:
- **Skill merge**: `git fetch upstream skill/channel-formatting && git merge upstream/skill/channel-formatting`. Brings in `src/text-styles.ts` (byte-identical to v1), `src/formatting.test.ts`, `container/skills/slack-formatting/SKILL.md`, plus the `.claude/skills/channel-formatting/SKILL.md` manifest.
- **No fork delta**. Drop v1's stashed copy (it's frozen reference only; never imported in v2).
- **Outbound wiring**: the skill ships with hookup into v2's outbound pipeline (per the SKILL.md description "wires channel-aware Markdown conversion into the outbound pipeline so Claude's responses render natively on each platform"). Verify post-merge.
...
1. **Skill-branch HEAD pinning**: same general OQ as CH1/CH2 — merge `upstream/skill/channel-formatting` HEAD at execution time, or pin to a known-good commit? Recommend **HEAD at execution time** unless a regression surfaces.
...
## Notes for Porter
- **One-line task**: skill merge + drop v1's stashed text-styles.ts from any import paths (`grep -r "from.*text-styles" src/ setup/` — should return zero hits after the merge since the skill provides the same file).
