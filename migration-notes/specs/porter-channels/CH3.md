...
> **✅ DONE (2026-06-06) — verification only, no code.** Resolution stands: take upstream `setup/whatsapp-auth.ts`, drop v1's `src/whatsapp-auth.ts`. Realized by the seed+repoint — `/add-whatsapp` fetches `setup/whatsapp-auth.ts` from `origin/channels` (upstream version: 10 `emitStatus`/`WHATSAPP_AUTH_QR` markers, 0 v1 `readline`/stdout-QR markers; `setup/status.ts` dep present). Verified: **nothing** in v2 imports the v1 `src/whatsapp-auth.ts` path (`grep` clean); v1's copy is frozen in `docs/v1-fork-reference/` only. The `getPlatformId` concern (decision-note era) is **obsolete**: it was a Baileys-6 workaround, and `/add-whatsapp` pins **Baileys 7.0.0-rc.9** (fixed natively) — both the v1 ref and current upstream show 0 `getPlatformId` refs. Minor cleanliness note (not acted on, outside CH3 scope): `v2-migration` trunk carries a stale copy of `setup/whatsapp-auth.ts` that the install fetch overwrites from `origin/channels`. Sections below = original decision record.
