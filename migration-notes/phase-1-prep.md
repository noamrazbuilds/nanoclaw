...
> **🔒 QUEUE LOCKED — 2026-05-19 (re-locked after U1/F6/U3 scope reduction).** Three Verifier passes (Scout, Verifier-1, Verifier-2) + upstream channel-skill parity audit 2026-05-19. 20 rows (U3 conditionally DROPPED — see row note). From this point: Porter agents draw scope from this queue and the documented hook points. Any further changes require explicit unlock + delta record below.
...
> - **2026-05-19 (unlock 2):** Revised **U1**, **F6**, **U3** after fetching upstream channel-skill remotes (`whatsapp-fork`, `telegram-fork`) and parity-checking against fork. `whatsapp/skill/voice-transcription` + `.claude/skills/use-local-whisper` cover ~85% of U1. `whatsapp/skill/reactions` covers ~75% of F6 AND ships a byte-identical `status-tracker.ts` (so U3 → DROP). Estimated effort reduction ~1–2 Porter-days. Each revised row carries an explicit **"Confirm at spec extraction"** clause — if Porter audit finds the parity claim wrong (upstream API divergence, broken imports against v2's module layout, etc.), the row reverts to its pre-revision scope. Re-locked same day.
...
| F6 | **Inbound reactions** (NEW — Verifier-2 2026-05-18, scope-reduced 2026-05-19) | listener in `src/channels/whatsapp.ts`/`telegram.ts`; storage in `src/db.ts` (`reactions` table); `react_to_message` MCP tool in `container/agent-runner/src/ipc-mcp-stdio.ts` | Porter-Features (cross-cuts Channels — coordinate with CH1/CH2) | Apply `whatsapp/skill/reactions` (covers reactions table, WhatsApp listener, `react_to_message` MCP tool, `container/skills/reactions/SKILL.md`, `scripts/migrate-reactions.ts`) + port **Telegram-channel reaction listener delta** from v1 (`docs/v1-fork-reference/src/channels/telegram.ts`, est. <50 LOC) | Scope-reduced 2026-05-19: upstream's `whatsapp/skill/reactions` skill provides ~75%. Reactions table schema is byte-identical to v1 (composite PK on `message_id+chat_jid+reactor_jid`, 4 indexes on message/reactor/emoji/timestamp). WhatsApp listener + IPC + MCP tool all wired upstream. **Telegram listener is fork-only** and still needs porting. **Confirm at spec extraction.** |
