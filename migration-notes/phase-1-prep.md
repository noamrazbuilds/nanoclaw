...
1. Visit `https://github.com/nanocoai/nanoclaw-whatsapp` → Fork → choose `noamrazbuilds`
2. Visit `https://github.com/nanocoai/nanoclaw-telegram` → Fork → choose `noamrazbuilds`
...
> **🔒 QUEUE LOCKED — 2026-05-18 (re-locked 2026-05-18 after C6 addition).** Three Verifier passes complete (Scout, Verifier-1, Verifier-2). 20 rows, all unhedged. From this point: Porter agents draw scope from this queue and the documented hook points. Any further changes require explicit unlock + delta record below.
...
|---|---|---|---|---|
| U1 | `transcription.ts` | `src/transcription.ts` | apply `whatsapp/skill/voice-transcription` skill OR re-implement OpenAI Whisper call against v2's `downloadInboundMedia` | **Blocks channels.** Verifier-2 2026-05-18: v2 has NO native voice→text. Full port required. Whisper API client preferred for parity. |
| U2 | `image.ts` (incl. inbound sticker handler) | `src/image.ts` + `src/channels/whatsapp.ts:333-346` | `downloadInboundMedia` in the fork of `nanoclaw-whatsapp` | **Blocks channels.** Includes the 30-min inbound sticker port (Decision 3). |
...
| F6 | **Inbound reactions** (NEW — Verifier-2 2026-05-18) | listener in `src/channels/whatsapp.ts`/`telegram.ts`; storage in `src/db.ts` (`reactions` table); `react_to_message` MCP tool in `container/agent-runner/src/ipc-mcp-stdio.ts` | Porter-Features (cross-cuts Channels — coordinate with CH1/CH2) | v2 reactions table migration + channel-skill listener + `mcp-tools/core.ts` register | v2's outbound reaction op exists, but **inbound is missing**: listener, `reactions` table, and `react_to_message` MCP tool are all fork-only. Used by `/add-reactions` skill + container `reactions` skill. Three pieces, one feature — Porter-Features owns; CH1/CH2 must emit the listener events. |
...
| ID | Customization | v1 source | v2 hook | Notes |
|---|---|---|---|---|
| U3 | `status-tracker.ts` | `src/status-tracker.ts` | WhatsApp adapter's inbound message handler + agent reply send path; depends on v2's outbound reaction op | **PORT** (Verifier-2 2026-05-18: flipped from "drop"). v2's heartbeat is liveness infrastructure, NOT UX per-message progress reactions. Fork uses WhatsApp reaction emojis as user-visible status; v2 has no equivalent. Real customization, not infra overlap. |
