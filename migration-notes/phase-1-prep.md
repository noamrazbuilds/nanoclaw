...
> **🔒 QUEUE LOCKED — 2026-06-02 (re-locked after Phase 2 divergence-shape audit; unlock 5).** Three Verifier passes (Scout, Verifier-1, Verifier-2) + upstream channel-skill parity audit 2026-05-19 (two passes) + Porter-Features per-tool audit + spec divergence-shape audit 2026-06-02. 20 rows (U3 conditionally DROPPED; F6 reduces to cherry-pick-only; F5 reduces to 2 of 10 tools — see row notes). From this point: Porter agents draw scope from this queue, the documented hook points, and the cherry-pick mechanism specified in each affected spec. Any further changes require explicit unlock + delta record below.
...
> - **2026-05-19 (unlock 4):** During F5 spec extraction, per-tool audit found 8 of 10 v1 MCP tools already exist in v2's `mcp-tools/` (all 6 scheduling tools, send_message, react_to_message). F5 reduces to porting 2 fork-only tools: `register_group` + `generate_image`. Also: queue's F5 row text mentioned `host_op` as an MCP tool — audit confirmed it's NOT an MCP tool; the host-ops surface (refresh_oauth/restart_service/rebuild_container) lives in `src/host-ops.ts` and is **U5c scope, not F5**. F5 row revised; U5c row clarified to make the host-ops surface explicit. Re-locked same day.
> - **2026-06-02 (unlock 5):** At Phase 2 Porter-spawn time, divergence-shape audit of every skill ref cited in specs revealed the "merge skill X into fork main" framing was wrong in six specs (U1, U2, F6, CH1, CH3, U5b). WhatsApp skill branches are 17-25 commits ahead of `whatsapp-fork/main` — but only 1-2 commits are actual features; the rest is "merge main into skill" noise. Naive `git merge` would pull in ~8K LOC of drift; the right tool is **`git cherry-pick <feature-commit>`**. Verified expected conflict shape per cherry-pick (one file each — mostly `package-lock.json` or agent-runner files) via `git merge-tree`. Also caught a real correctness bug in U1: `telegram-fork/main:src/channels/telegram.ts:311` already has a bare `bot.on('message:voice', ...)` placeholder handler (stores `[Voice message]` + fileId, no download, no transcription); U1's scope is to **replace its body** to call `transcribeAudioBuffer`, not to add a new registration. Six specs revised inline with concrete cherry-pick commit hashes (`dfc23a2` image-vision, `c054cbc` voice-transcription, `a23e372` reactions, `e87d15d` channel-formatting). Re-locked same day. No row scope changes — only mechanism fixes + the U1 Telegram correctness fix.
...
| F5 | Custom agent tools surface (per-tool audit 2026-05-19: PORT `register_group` + `generate_image`; DROP rest as duplicates of v2 native) | `container/agent-runner/src/ipc-mcp-stdio.ts:413, 467` | Porter-Features | new files under `container/agent-runner/src/mcp-tools/` | **Reduced 2026-05-19 (unlock 4):** per-tool audit found 8 of 10 v1 tools already in v2 — all 6 scheduling tools (`scheduleTask`, `listTasks`, `pauseTask`, `resumeTask`, `cancelTask`, `updateTask`), `sendMessage`, `addReaction`. Only `register_group` (chat-group registration, main-group-only) + `generate_image` (LiteLLM gpt-image-2) are fork-only. ~80 LOC per tool. **`host_op` referenced in original row text is NOT an MCP tool** — host-ops surface (refresh_oauth/restart_service/rebuild_container) lives in `src/host-ops.ts` and is **U5c scope**. See spec `migration-notes/specs/porter-features/F5.md` for per-tool audit table. |
...
| U5c | `host-ops.ts` (host-ops surface: refresh_oauth, restart_service, rebuild_container) | `src/host-ops.ts` + `src/ipc.ts:16` (executeHostOp/isValidHostOp imports) | (unbundled) | Clarified 2026-05-19 (unlock 4): this row owns the host-ops surface that F5's queue text incorrectly attributed to MCP tools. The agent invokes host ops via IPC from the container (not via an MCP tool named `host_op`); the host's `executeHostOp` consumes the IPC and runs the predefined op. Per-op allowlist is the security control. |
