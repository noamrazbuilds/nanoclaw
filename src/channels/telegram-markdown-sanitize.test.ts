...
describe('sanitizeTelegramLegacyMarkdown', () => {
  ...
  it('strips a stray/unpaired backtick (would 400 + silently drop the message)', () => {
    expect(sanitizeTelegramLegacyMarkdown('use the `config value to set it')).toBe('use the config value to set it');
  });

  it('leaves backticks BALANCED when a code fence is split across a chunk boundary', () => {
    // The bridge chunks long messages; a ``` fence can land unbalanced in one
    // chunk. The safety guarantee is balanced backticks (Telegram 400s on an
    // UNTERMINATED code entity, not an even/empty one) + content preserved.
    const out = sanitizeTelegramLegacyMarkdown('here is the start ```js\nconst x = 1');
    expect(out).toContain('const x = 1');
    expect((out.match(/`/g) ?? []).length % 2).toBe(0); // balanced → won't 400
  });

  it('preserves a properly paired inline code span', () => {
    expect(sanitizeTelegramLegacyMarkdown('the `gws_run` tool')).toBe('the `gws_run` tool');
  });
});
