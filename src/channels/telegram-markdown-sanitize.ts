...
export function sanitizeTelegramLegacyMarkdown(input: string): string {
  ...
  // Stray backticks. CODE_PATTERN above already extracted every *paired* inline/
  // fenced code span into a placeholder, so any backtick still in `text` is
  // unbalanced — a lone `` ` ``, or half of a code fence split across a chunk
  // boundary (the bridge chunks long messages). Telegram's legacy Markdown
  // rejects an unterminated code entity with HTTP 400 and the message is then
  // silently dropped after retries, so strip the strays. (Backticks inside the
  // restored code placeholders are literal to Telegram and untouched.)
  text = text.replace(/`/g, '');
