...
import { ARENA_LITELLM_URL, ARENA_LITELLM_KEY, LOCAL_MODEL_TIMEOUT_MS, ARENA_SYSTEM_PROMPT } from './arena-config.js';
...
export async function callModel(
  ...
  const doFetch = () =>
    fetch(url, {
      ...
      headers: {
        ...
        ...(ARENA_LITELLM_KEY ? { Authorization: `Bearer ${ARENA_LITELLM_KEY}` } : {}),
      },
      body: JSON.stringify(payload),
      signal: botConfig.local ? AbortSignal.timeout(LOCAL_MODEL_TIMEOUT_MS) : AbortSignal.timeout(120_000),
    });

  const response = botConfig.local ? await runOnLocalQueue(doFetch) : await doFetch();

  if (!response.ok) {
    const body = await response.text().catch(() => '');
    throw new Error(`LiteLLM ${response.status} for ${botConfig.model}: ${body.slice(0, 500)}`);
  }
  ...
  const content = choice?.message?.content || (choice?.message as Record<string, unknown>)?.reasoning_content;
  if (!content || typeof content !== 'string') {
    throw new Error(`Empty response from ${botConfig.model}`);
  }

  log.debug('Arena model call complete', {
    bot: botConfig.id,
    model: botConfig.model,
    latencyMs: latency,
