 ...
 * trivially "wins".
 */
export function buildDailyAggregates(rows: GradedLogRow[], periodStart: string, periodEnd: string): ArenaAggregate[] {
  ...
  for (const r of rows) {
    ...
    if (!scoresBySession.has(r.session_id)) scoresBySession.set(r.session_id, []);
  ...
  for (const [, sessionRows] of scoresBySession) {
    ...
    for (const r of sessionRows) {
      competitiveSessionsByModel.set(r.model, (competitiveSessionsByModel.get(r.model) ?? 0) + 1);
    }
 ...
 */
export function runDailyAggregation({ days = 14 }: { days?: number } = {}): void {
  ...
  log.info('Arena aggregates recomputed', { days, written });
}
