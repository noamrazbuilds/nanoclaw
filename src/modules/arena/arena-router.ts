...
export function classifyMessage(
  ...
  if (message.reply_to_message?.from?.id) {
    ...
    if (targetBot) {
      log.debug('Arena reply-to routing', {
        botId: targetBot.id,
  ...
  if (message.text && message.entities) {
    ...
    if (mentions.length > 0) {
      ...
      if (targetBotIds.length > 0) {
        log.debug('Arena targeted routing', { targetBotIds, mentions });
...
export function extractRating(newReaction: Array<{ type: string; emoji?: string }> | undefined): number {
