...
export async function runDailyGrading(): Promise<void> {
  ...
  if (sessions.length === 0) {
    log.info('Arena grading: no ungraded sessions');
    return;
  }

  log.info('Arena grading: starting', { count: sessions.length });
...
export async function gradeOneSession(sessionId: string): Promise<'graded' | 'failed'> {
  ...
  try {
    ...
    log.info('Arena grading: session graded', { sessionId, gradesCount: grades.length });
    return 'graded';
  } catch (err) {
    log.error('Arena grading: failed', { sessionId, err });
    arenaDb.updateSessionStatus(sessionId, 'failed');
    return 'failed';
  }
}

async function gradeSessionWithRetry(sessionId: string, responses: ArenaLog[]): Promise<ArenaGrade[]> {
  for (let attempt = 0; attempt < GRADER_MAX_RETRIES; attempt++) {
    try {
      ...
      const parsed = parseGraderJson(content) as Parameters<typeof mapGraderOutput>[2];
      ...
      log.warn('Arena grader retry', { attempt, delay, err });
