...
function addSubScore(bucket: { sum: number; n: number }, value: number | null): void {
...
export async function generateReport(options: { days: number }): Promise<string> {
  const now = new Date();
  ...
  for (const a of aggregates) {
    ...
    r.responses += n;
    if (a.avg_overall_score != null) r.weightedScoreSum += a.avg_overall_score * n;
  ...
  for (let i = 0; i < rollups.length; i++) {
    const m = rollups[i];
    const rank = i + 1;
    const score = m.responses > 0 ? (m.weightedScoreSum / m.responses).toFixed(1) : 'N/A';
    const winRate =
      m.winShareDenominator > 0 ? `${((m.winShareNumerator / m.winShareDenominator) * 100).toFixed(0)}%` : 'N/A';
    const cost = m.weightedCostResponses > 0 ? `$${(m.weightedCostSum / m.weightedCostResponses).toFixed(4)}` : 'N/A';
    const latency = m.responses > 0 ? `${(m.weightedLatencySum / m.responses).toFixed(0)}ms` : 'N/A';
  ...
  // User engagement.
  const anyEngagement = rollups.some((m) => m.replyRateNumerator > 0 || m.ratingResponses > 0);
  if (anyEngagement) {
    report += `\n**User Engagement:**\n`;
  ...
  log.info('Arena report generated', { days: options.days, models: rollups.length });
  return report;
}
